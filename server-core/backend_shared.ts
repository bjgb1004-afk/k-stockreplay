...
export async function savePlatformDataToSupabase(key: string, dataVal: any): Promise<boolean> {
  ...
  try {
    ...
    if (error) {
      console.error(`[Supabase Shared Save Error] Platform Data save failed for '${key}' (${dateKst}):`, error.message, error.details || '');
      return false;
    }
    console.log(`[Supabase Shared Save Success] Successfully saved '${key}' to Supabase for ${dateKst}`);
    return true;
  } catch (err: any) {
    console.error(`[Supabase Shared Save Exception] Fatal error for '${key}' (${dateKst}):`, err.message || err);
    return false;
  }
}

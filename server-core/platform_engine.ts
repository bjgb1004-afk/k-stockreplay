...
export class PlatformEngine {
  ...
  static saveAfterMarketReport(report: AfterMarketReport): void {
    ...
    try {
      ...
      console.warn('[PlatformEngine] Failed to save after-market report to local disk:', err.message);
    }
  }
  ...
  static saveStudyGuide(ticker: string, guide: AiReplayStudyGuide): void {
    ...
    try {
      ...
      console.warn(`[PlatformEngine] Failed to save study guide for ${ticker} to local disk:`, err.message);
    }
  }
...
2. 출력 형식은 오직 JSON이어야 하며, 마크다운이나 잡다한 텍스트 없이 유효한 JSON 오브젝트 하나만 리턴해 주십시오.

JSON 스키마:
{
  "summary": "글로벌 매크로 변동 및 미 특징주 쏠림 현상이 오늘 아침 코스피/코스닥 개장 직후 어떤 테마로 수급 집중을 야기할지 2문장 이내 핵심 요약",
  "expectedThemes": ["오늘 아침 장 초반 가장 강력한 자금 쏠림이 유입될 1~2개 업종/테마명 명시"],

...
async function savePlatformDataToSupabase(key: string, dataVal: any): Promise<boolean> {
  const dateKst = dataVal?.date || getTodayKSTString();
  ...
  // Storage backup (optional, try-catch protected)
  ...
  // Use Admin Client to bypass RLS for server-side writes
  const supabase = getSupabaseAdmin() || getSupabase();
  if (!supabase) {
    console.error(`[Supabase Save Error] Supabase client not initialized for key: ${key}`);
    return false;
  }
  ...
  try {
    ...
    if (error) {
      console.error(`[Supabase Save Error] Platform Data save failed for '${key}' (${dateKst}):`, error.message, error.details || '');
      return false;
    }

    console.log(`[Supabase Save Success] Successfully saved '${key}' to Supabase for ${dateKst}`);
    return true;
  } catch (err: any) {
    console.error(`[Supabase Save Exception] Fatal error for '${key}' (${dateKst}):`, err.message || err);
    return false;
  }
}
...
CREATE TABLE kstock_platform_data (
  ...
  app.post('/api/cron/pre-market', async (req, res) => {
    ...
    try {
      ...
      if (!isSaved) {
        throw new Error('Supabase에 장전 브리핑을 저장하지 못했습니다. 서비스 로그를 확인하십시오.');
      }
...
<p>지수 차트가 5일 및 20일 이동평균선을 이탈하며 대량 거래량을 동반한 역배열 급락 파동을 전개할 때는, 아무리 강력해 보이는 호재 공시가 돌출하더라도 공격적 추격 매수를 원천 봉쇄하고 보유 자산의 최소 60% 이상을 안정적인 원화 현금 상태로 피신해야 합니다. 주식 시장에서는 매일 새로운 주도주와 강렬한 테마 파동이 끊임없이 탄생하므로 조급함을 버려야 합니다. 준비된 원금을 안전하게 축적해 둔 소수의 스마트 머니만이 다음 수급 대상승 기회가 왔을 때 계좌를 수십 배로 폭등시키는 단 과실을 쟁취할 수 있음을 상기하시며, 늘 평정심을 유지하는 원칙 트레이딩을 유지하시기를 바랍니다.</p>`;
  ...
  const handleKisTokenRefresh = async (req: express.Request, res: express.Response) => {
    ...
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
      console.warn('[KIS Token Refresh] SUPABASE_SERVICE_ROLE_KEY missing, falling back to ANON_KEY. Token save might fail if RLS is enabled.');
    }
...
[기준 시점: DART 정기 공시 및 FnGuide 최근 데이터 기준]`
  ...
  app.all('/api/cron/market-close', verifyCronAuth, async (req, res) => {
    try {
      ...
      // Save to Supabase (Primary storage for production)
      const isSaved = await savePlatformDataToSupabase('afternoon_report', report);
      ...
      if (!isSaved) {
        throw new Error('Supabase에 장마감 리포트를 저장하지 못했습니다. 서비스 로그를 확인하십시오.');
      }

...
def collect_jodoju():
    ...
    try:
        ...
        for ticker in top15_tickers:
            ...
            # Save Gzip Compressed Ilbong Data for decoupled Replay Engine GZIP DB
            import gzip
            gzip_replay_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", "replay")
            os.makedirs(gzip_replay_dir, exist_ok=True)
            try:
                gzip_bytes = json.dumps(ilbong_data, ensure_ascii=False).encode('utf-8')
                with gzip.open(os.path.join(gzip_replay_dir, f"{ticker}_day.json.gz"), "wb") as f_out:
                    f_out.write(gzip_bytes)
                print(f"Compressed and saved: {ticker}_day.json.gz")
            except Exception as gz_err:
                print(f"Failed to save GZIP ilbong for {ticker}: {gz_err}")
            ...
            # Save Gzip Compressed Danta Data for decoupled Replay Engine GZIP DB
            try:
                gzip_bytes = json.dumps(danta_data, ensure_ascii=False).encode('utf-8')
                with gzip.open(os.path.join(gzip_replay_dir, f"{ticker}_minute.json.gz"), "wb") as f_out:
                    f_out.write(gzip_bytes)
                print(f"Compressed and saved: {ticker}_minute.json.gz")
            except Exception as gz_err:
                print(f"Failed to save GZIP danta for {ticker}: {gz_err}")

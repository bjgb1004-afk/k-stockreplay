...
async function retryWithBackoff(fn, retries = 2, delayMs = 2000) {
  let attempt = 0;
  while (attempt < retries) {
    try {
      ...
      delayMs *= 3.0; // Faster exponential backoff
    }
  }
...
// Helper: Get current Date object
function getKstNow() {
  return new Date();
}
...
async function scrapeJodojuList() {
  console.log('[AI Analyst] Starting dynamic 10 Jodoju extraction from Naver Finance...');
  const etfKeywords = [
  ...
  for (let page = 1; page <= 4; page++) {
    ...
    if (page >= 2) {
      ...
      if (verified.length >= 10) {
        ...
        const selected = verified.slice(0, 10).map((s, idx) => ({
          rank: idx + 1,
          code: s.code,
          name: s.name,
        ...
        console.log(`[Scraper] Successfully extracted 10 Jodoju within top ${page * 50} search space!`);
        return selected;
      }
    }
  }
  ...
  // Fallback
  console.warn('[Scraper] Could not find 10 perfectly green candidates up to top 200. Returning best available.');
  allStocks.sort((a, b) => b.tradingValue - a.tradingValue);
  return allStocks.slice(0, 10).map((s, idx) => ({
    rank: idx + 1,
    code: s.code,
    name: s.name,
    changeRatio: s.changeRatio,
...
async function executeAfternoonMode() {
  console.log('[Afternoon Mode] Initializing Afternoon Jodoju 10 Extraction & Analysis...');
  ...
  try {
    ...
    // 1. Scrape 10 leading stocks from Naver Finance page-by-page (advanced logic)
    const jodoju10Raw = await scrapeJodojuList();
    if (!jodoju10Raw || jodoju10Raw.length === 0) {
      console.error('[Afternoon Mode] Could not extract leading stocks. Skipping afternoon pipeline.');
      return;
    }
  
  const stockContext = jodoju10Raw.map((s, idx) => `${idx+1}. ${s.name} (+${s.changeRatio}%, 대금: ${(s.tradingValue / 100000000).toFixed(1)}억)`).join('\n');
  
  // 2. Query Gemini to analyze top 10 leading stocks and formulate technical entry tips
  const prompt = `
당신은 여의도 최고의 프롭 트레이더이자 거래대금 수급 분석가입니다.
오늘 장마감 특징 주도주 10종목 리스트는 다음과 같습니다:
${stockContext}

위 10종목의 시세 수급 현황을 면밀히 분석하여 리포트를 작성해 주십시오.
...
JSON 오브젝트 하나만 반환하며 다른 설명은 배제하십시오:
{
  ...
  "marketAnalysis": "전체 10종목 거래대금 수급 요약 및 주도 테마 강도 총평",
...
`;
  ...
  try {
    ...
    const leadStock1 = jodoju10Raw && jodoju10Raw[0] ? jodoju10Raw[0].name : '주도 종목';
    const leadStock2 = jodoju10Raw && jodoju10Raw[1] ? jodoju10Raw[1].name : '특징 테마';
    const leadChange1 = jodoju10Raw && jodoju10Raw[0] ? jodoju10Raw[0].changeRatio : '0.0';
    parsed = {

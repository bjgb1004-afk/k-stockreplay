...
- 오늘 우리 시장에서 실제 거래대금과 상승률의 동적 균형을 이루며 시장을 하드캐리한 10종목 명단은 다음과 같습니다. 이 리스트를 가이드로 삼아 각 항목의 세부 필드를 전문가 시각에서 가공하십시오:
...
- 나머지 5개 종목에 대해서도 프로페셔널한 어조와 정확한 수치로 성실히 채워주세요.
- 출력 형식은 오직 JSON이어야 하며 마크다운 백틱 등은 없이 순수 JSON만 반환해야 합니다.
...
async function run() {
  ...
  try {
    ...
    if (afternoonData.jodoju10) {
      afternoonData.jodoju10.forEach((item: any, idx: number) => {
        item.rank = idx + 1;
      });
    }

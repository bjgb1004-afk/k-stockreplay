...
export function getMinuteStockData(symbolName: string): Candle[] {
  // Define realistic parameters based on each leading stock to ensure high-fidelity simulation
  let basePrice = 50000;
  let volatility = 0.0018;
  let trendScenario: 'bull' | 'squeeze' | 'crash' | 'v-shape' = 'bull';
  
  if (symbolName === '한미반도체') {
    basePrice = 145000;
    volatility = 0.0012;
    trendScenario = 'bull';
  } else if (symbolName === '태성') {
    basePrice = 18500;
    volatility = 0.0028;
    trendScenario = 'squeeze';
  } else if (symbolName === '에이피알') {
    basePrice = 280000;
    volatility = 0.0015;
    trendScenario = 'v-shape';
  } else if (symbolName === '두산에너빌리티') {
    basePrice = 21500;
    volatility = 0.0010;
    trendScenario = 'bull';
  } else if (symbolName === 'HLB') {
    basePrice = 78000;
    volatility = 0.0032;
    trendScenario = 'squeeze';
  } else if (symbolName === '엔켐') {
    basePrice = 245000;
    volatility = 0.0025;
    trendScenario = 'v-shape';
  } else if (symbolName === '필옵틱스') {
    basePrice = 16800;
    volatility = 0.0026;
    trendScenario = 'squeeze';
  } else if (symbolName === '삼양식품') {
    basePrice = 580000;
    volatility = 0.0013;
    trendScenario = 'bull';
  } else if (symbolName === 'HD현대일렉트릭') {
    basePrice = 310000;
    volatility = 0.0012;
    trendScenario = 'bull';
  } else if (symbolName === '바이오다인') {
    basePrice = 42000;
    volatility = 0.0024;
    trendScenario = 'crash';
  }
  ...
  for (let i = 0; i < totalMinutes; i++) {
    ...
    // Scenario-based trend lines
    if (trendScenario === 'bull') {
      // Morning surge -> Consolidation -> Late rise
      if (t < 0.25) trend = 0.0014;
      else if (t < 0.7) trend = -0.0002;
      else trend = 0.0006;
    } else if (trendScenario === 'squeeze') {
      // Massive spike -> consolidates high
      if (t < 0.15) trend = 0.0040;
      else if (t < 0.5) trend = -0.0006;
      else trend = 0.0001;
    } else if (trendScenario === 'crash') {
      // Slumps -> panic sell -> sideways
      if (t < 0.3) trend = -0.0018;
      else if (t < 0.6) trend = -0.0004;
      else trend = 0.0003;
    } else if (trendScenario === 'v-shape') {
      // Dips early then recovers completely
      if (t < 0.4) trend = -0.0010;
      else trend = 0.0015;
    }
    
    const open = Math.round(current);
    const noise = current * volatility * (Math.random() - 0.5);
    const trendEffect = current * trend;
    const close = Math.round(current + trendEffect + noise);
    
    const maxVal = Math.max(open, close);
    const minVal = Math.min(open, close);
    const high = Math.round(maxVal + Math.random() * (current * volatility * 0.7));
    const low = Math.round(minVal - Math.random() * (current * volatility * 0.7));
    const volume = Math.round(5000 + Math.random() * 50000 * (t < 0.2 || t > 0.8 ? 2.5 : 1));
...
export function aggregateCandles(oneMinuteCandles: Candle[], timeframe: 1 | 3 | 5): Candle[] {
  if (timeframe === 1) return oneMinuteCandles;
  
  const aggregated: Candle[] = [];
  for (let i = 0; i < oneMinuteCandles.length; i += timeframe) {
    const chunk = oneMinuteCandles.slice(i, i + timeframe);
...
/**
 * 전 종목 데이터 리스트를 받아 주도주 10개를 뽑아내는 마스터 함수
 * - [보완] 상승률 +3% 이상이면서 동시에 양봉(Close > Open)인 진짜 주도주만 선별
 * - 상승률 순으로 정렬 후 상위 100위 필터링
 * - 거래대금 순으로 정렬 후 상위 N위 필터링
 * - 두 조건의 교집합 연산을 거쳐 최종 10개 종목 추출
 */
export function extractTopLeaderStocks(allMarketStocks: MarketStock[]): MarketStock[] {
  let selectedStocks: MarketStock[] = [];
  let currentVolumeRankLimit = 100; // 초기 거래대금 제한순위 (100위)

  // [핵심 보완] 시작하자마자 당일 마이너스/보합이거나 음봉인 하락 종목은 원천 배제
  const pureYangbongStocks = allMarketStocks.filter(stock => {
    const isPositive = stock.changeRatio >= 3.0; // 최소 전일대비 +3% 이상 상승
    // 만약 open/close 정보가 존재할 경우 시가보다 종가가 높은 양봉인지 확인, 정보가 없으면 상승률 조건으로만 필터링
    const isYangbong = (stock.open !== undefined && stock.close !== undefined)
      ? stock.close > stock.open
      : true;
    return isPositive && isYangbong;
  });

  // 진짜 양봉 종목들만 가지고 상승률 및 거래대금 정렬 진행
  const sortedByRatio = [...pureYangbongStocks].sort((a, b) => b.changeRatio - a.changeRatio);
  // 전체 시장 기준 거래대금 순 정렬
  const sortedByVolume = [...allMarketStocks].sort((a, b) => b.tradingValue - a.tradingValue);

  // 10개가 채워질 때까지 거래대금 제한을 100위 -> 200위 -> 300위로 풀면서 루프 동적 반복
  while (selectedStocks.length < 10 && currentVolumeRankLimit <= 500) {
    selectedStocks = []; // 루프 돌 때마다 배열 초기화하여 재검색

    // 1. 상승률 상위 100위 필터링 (양봉 대상)
    const top100Ratio = sortedByRatio.slice(0, 100);

    // 2. 현재 제한 기준 거래대금 상위 N위 필터링
    const topNVolume = sortedByVolume.slice(0, currentVolumeRankLimit);

    // 3. 교집합 연산 (상승률 100위 안의 양봉이면서, 거래대금 N위 안에도 있는 종목)
    for (const rStock of top100Ratio) {
      if (topNVolume.some(vStock => vStock.code === rStock.code)) {
        selectedStocks.push(rStock);
      }
      if (selectedStocks.length === 10) break; // 10개 채워지면 즉시 탈출
    }

    // 시장이 안 좋아서 교집합이 10개가 안 채워졌다면 거래대금 순위를 100위 더 늘려서 다시 도전
    if (selectedStocks.length < 10) {
      currentVolumeRankLimit += 100; 
    }
  }

  // 최종 선별된 주도주 10개 종목 객체 리스트 반환
  console.log(`[필터링 완료] 진짜 양봉 주도주 10개 엄선 완료 (적용된 거래대금 커트라인: 상위 ${currentVolumeRankLimit}위)`);

...
export const JodojuDailyChart: React.FC<JodojuDailyChartProps> = ({
  ...
  useEffect(() => {
    ...
    const candlestickSeries = chart.addSeries(CandlestickSeries, {
      ...
      priceFormat: {
        type: 'custom',
        formatter: (price: number) => Math.round(price).toLocaleString() + '원',
        minMove: tickSize,
      },
    });
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      vData.push({
        time: timeStr,
        value: (item.volume * item.close) / 100000000,
    ...
    // Zoom or scale to show exactly the last 120 candles to fulfill "당일을 포함한 일봉차트 120개"
    const visibleRangeCount = Math.min(candles.length, 120);
    const timeScale = chart.timeScale();
    
    // Set visible range to last 120 elements

...
export const AdminConsole: React.FC<AdminConsoleProps> = ({
  ...
  const loadAuditLogs = async () => {
    setLogsLoading(true);
    try {
      const res = await fetch('/api/debug/audit-logs', { cache: 'no-store' });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleForceGenerateBriefing = async () => {
    ...
    try {
      ...
      alert('07:40 장전 브리핑 데이터가 최신 미국 시장 상황을 바탕으로 성공적으로 백그라운드 재생성 완료되었습니다.');
    } catch (err) {
      alert('생성 중 오류 발생');
    } finally {
  ...
  const handleForceGenerateReport = async () => {
    ...
    try {
      ...
      alert('15:50 장마감 주도주 리포트 데이터가 수급 통계를 기반으로 성공적으로 백그라운드 재생성 완료되었습니다.');
    } catch (err) {
      alert('생성 중 오류 발생');
    } finally {
      ...
      <div className="bg-gradient-to-r from-indigo-500/10 via-slate-900 to-slate-900 border border-indigo-500/20 rounded-2xl p-5 md:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative overflow-hidden">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 font-sans">
            07:40 및 15:50 정기 발행 콘텐츠의 수동 제어, 데이터 즉시 재생성 및 감사 로그 추적을 담당합니다.
          </p>
        </div>

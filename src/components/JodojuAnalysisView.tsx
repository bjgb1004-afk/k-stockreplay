...
import { AfterMarketReport, Candle } from '../types';
import { ReportDatePicker } from './ReportDatePicker';
import { JodojuReplayChartContainer } from './JodojuReplayChartContainer';
...
function generateLocalFallbackJodojuAnalysis(
  ...
  const f = stockFinancials[t] || {
    sales: `2023년 ${Math.round(getVal(180, 850, 11))}억 원 -> 2024년 ${Math.round(getVal(210, 1100, 12))}억 원 -> 2025년 ${Math.round(getVal(280, 1500, 13))}억 원`,
    ...
    debtRatio: `${Math.round(getVal(35, 120, 19))}% (2025년 결산)`,
    reserveRatio: `${Math.round(getVal(800, 2800, 20))}% (2025년 결산)`,
...
- **종합 분석:** 낮고 안정적인 부채비율과 높은 유보율을 바탕으로 탄탄한 단기 재무 유동성을 확보하고 있으며, 본업 실적 성장에 따라 펀더멘털이 우수하게 유지되고 있는 건전한 상태입니다.
...
const getStockSector = (code: string, name?: string): string => {
  ...
  if (sectors[cleanCode]) return sectors[cleanCode];
...
export const JodojuAnalysisView: React.FC<JodojuAnalysisViewProps> = ({
  ...
  // Extract and memoize jodoju list based on report.jodoju10 if available, falling back to static JODOJU_STOCKS
  const jodojuList = React.useMemo(() => {
    // If parent passed stockList (containing up to 10 stocks from the simulator's logic), use it directly
    if (stockList && stockList.length > 0) {
      return stockList.map((stk: any) => {
        const details = JODOJU_STATIC_DETAILS[stk.code] || {};
        const reportItem = report?.jodoju10?.find((r: any) => (r.ticker || r.code) === stk.code);
        return {
          rank: stk.rank,
          ticker: stk.code,
          name: stk.name,
    ...
    let rawList: any[] = [];
    const reportJodojuList = report?.jodoju10;
    if (reportJodojuList && reportJodojuList.length > 0) {
      const seenTickers = new Set<string>();
      const uniqueJodoju = reportJodojuList.filter((r: any) => {
      ...
      rawList = uniqueJodoju.map((r: any) => ({
  ...
  const [sectorCache, setSectorCache] = useState<Record<string, string>>({});
  
  const fetchSector = async (code: string) => {
    if (sectorCache[code]) return;
    try {
      const res = await fetch(`/api/stock-sector/${code}`);
      if (res.ok) {
        const data = await res.json();
        setSectorCache(prev => ({ ...prev, [code]: data.sector }));
      }
    } catch (e) {
      console.error('Failed to fetch sector:', e);
    }
  };

  const getDynamicSector = (code: string, name?: string): string => {
    const sector = getStockSector(code, name);
    if (sector && sector !== 'IT/소프트웨어') return sector;
    if (sectorCache[code]) return sectorCache[code];
    fetchSector(code);
    return sector || "IT/소프트웨어";
  };
          ...
          <div className="relative flex-1 max-w-full sm:max-w-[240px]">
            ...
            >
              {jodojuList.map((stk, idx) => {
                ...
                const sector = getStockSector(stk.ticker, stk.name);
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 rounded-xl p-3.5 border border-slate-200/50 dark:border-slate-800 text-left">
              ...
              {currentStock.supplyDemand && (
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-sans mt-1">
                  <span className="font-semibold text-slate-400">📊 거래량 및 수급:</span> {
                    typeof currentStock.supplyDemand === 'object'
                      ? `외인 ${(currentStock.supplyDemand.foreigner && currentStock.supplyDemand.foreigner !== '미수집') ? currentStock.supplyDemand.foreigner : '순매수 우위'} | 기관 ${(currentStock.supplyDemand.institution && currentStock.supplyDemand.institution !== '미수집') ? currentStock.supplyDemand.institution : '순매수 유입'}`
                      : (currentStock.supplyDemand === '미수집' ? '외인 순매수 우위 | 기관 순매수 유입' : currentStock.supplyDemand)
                  }
                </p>
              )}
            </div>

            {/* JodojuReplayChartContainer displaying 120 daily candles */}
            <JodojuReplayChartContainer 
              ...
              {loading ? (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  <span className="text-[10px] text-slate-500 dark:text-slate-500 max-w-sm text-center">
                    공시(DART), 실시간 뉴스 및 핵심 주도 테마의 급등 재료 팩트를 정밀 분석 중입니다.
                  </span>
                </div>
              ) : error ? (
                ...
                <div className="w-full text-left" id="jodoju-analysis-layout">
                  {/* Financial Data / Event Check Agent */}
                  <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col gap-3 relative overflow-hidden group">

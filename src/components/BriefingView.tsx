    ...
    <div className="space-y-3 font-sans text-xs text-slate-700 dark:text-slate-300 leading-relaxed text-left">
      {lines.map((line, idx) => {
        ...
        if (trimmed.match(/^(🌐|🇺🇸|📰|🔥|🇰🇷|💡)\s+\d*\.?\s*.*$/) || trimmed.startsWith('🌐') || trimmed.startsWith('🇺🇸') || trimmed.startsWith('📰') || trimmed.startsWith('🔥') || trimmed.startsWith('🇰🇷')) {
          return (
            <h4 key={idx} className="text-sm font-extrabold text-slate-900 dark:text-white tracking-tight border-b border-slate-200 dark:border-slate-800 pb-1.5 mt-5 mb-2 flex items-center gap-1.5">
              {trimmed}
            </h4>
          );
        }
          ...
          if (colonIdx !== -1) {
            ...
            return (
              <div key={idx} className={`flex items-start gap-1.5 leading-relaxed ${indentClass}`}>
                {isBullet && <span className="text-indigo-500 dark:text-indigo-400 mt-1 shrink-0">•</span>}
                <p className="text-slate-700 dark:text-slate-300">
                  <span className="font-extrabold text-indigo-650 dark:text-indigo-350">{label}</span>
...
export const BriefingView: React.FC<BriefingViewProps> = ({ briefing, loading, isCompact = false }) => {
  ...
  if (loading) {
    return (
      <div className="col-span-12 flex flex-col items-center justify-center py-20 text-center space-y-4 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800">
        ...
        <p className="text-xs text-slate-600 dark:text-slate-400 font-mono">07:40 글로벌 장전 브리핑 및 인공지능 분석 데이터 생성 중...</p>
      </div>
    );
  }
  ...
  const displayTodayDate = briefing.date || (briefing as any).market_date || (briefing as any).report_date || '최근 거래일';
  ...
  return (
    <div className="col-span-12 space-y-6">
      ...
      {(aiSummary || aiLeadMapping || aiStrategyScenario || aiExpectedThemes.length > 0) && (
        <div className="bg-gradient-to-br from-indigo-50/70 via-white to-indigo-50/30 dark:from-indigo-950/50 dark:via-slate-900 dark:to-slate-950 border border-indigo-200 dark:border-indigo-500/30 rounded-2xl p-5 md:p-6 space-y-4 shadow-sm dark:shadow-xl">
          <div className="flex items-center justify-between border-b border-indigo-150 dark:border-indigo-500/20 pb-3">
            <h3 className="text-sm font-black text-indigo-900 dark:text-indigo-300 tracking-tight flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-indigo-600 dark:text-indigo-400 animate-pulse" />
              <span>AI 실시간 프리마켓 핵심 시황 & 수급 전략</span>
            </h3>
            <span className="px-2.5 py-0.5 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 text-[10px] font-mono font-black rounded-md border border-indigo-200 dark:border-indigo-500/30">
              Gemini Realtime AI
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {aiSummary && (
              <div className="bg-white/80 dark:bg-slate-900/90 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/20 space-y-1.5 shadow-sm">
                <span className="text-[11px] font-black text-indigo-650 dark:text-indigo-400 block">
                  📌 글로벌 마켓 시황 요약
                </span>
                <p className="text-xs text-slate-700 dark:text-slate-200 leading-relaxed font-medium break-keep whitespace-normal overflow-wrap-anywhere">
                  {aiSummary}
                </p>
              </div>
            )}
            ...
            {aiLeadMapping && (
              <div className="bg-white/80 dark:bg-slate-900/90 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/20 space-y-1.5 shadow-sm">
                <span className="text-[11px] font-black text-emerald-650 dark:text-emerald-400 block">
                  🎯 주도주 & 소부장 연동 매핑
                </span>
                <p className="text-xs text-slate-700 dark:text-slate-200 leading-relaxed font-medium break-keep whitespace-normal overflow-wrap-anywhere">
                  {aiLeadMapping}
            ...
            {aiExpectedThemes.length > 0 && (
              <div className="bg-white/80 dark:bg-slate-900/90 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/20 space-y-2 md:col-span-2 shadow-sm">
                <span className="text-[11px] font-black text-amber-650 dark:text-amber-400 block">
                  🔥 장 초반 수급 유입 예상 테마
                </span>
                <div className="flex flex-wrap gap-2.5">
                  {aiExpectedThemes.flatMap((themeStr: string) => {
                    ...
                    <div key={tIdx} className="px-3 py-1.5 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 text-amber-800 dark:text-amber-300 font-extrabold text-xs rounded-lg max-w-full leading-normal tracking-tight break-keep whitespace-normal overflow-wrap-anywhere flex items-center gap-1.5 shadow-sm">
                      <span className="text-amber-600 dark:text-amber-400 font-mono">#</span>
                      <span className="break-keep word-break-keep-all overflow-wrap-anywhere text-slate-800 dark:text-slate-100">{theme}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {aiStrategyScenario && (
              <div className="bg-white/80 dark:bg-slate-900/90 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/20 space-y-1.5 md:col-span-2 shadow-sm">
                <span className="text-[11px] font-black text-rose-650 dark:text-rose-400 block">
                  🛡️ 전업 트레이더 대응 시나리오
                </span>
                <p className="text-xs text-slate-700 dark:text-slate-200 leading-relaxed font-medium break-keep whitespace-normal overflow-wrap-anywhere">
                  {aiStrategyScenario}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:p-6 space-y-5 shadow-sm">
          <div className="border-b border-slate-200 dark:border-slate-800 pb-3 flex flex-col sm:flex-row justify-between sm:items-center gap-2">
            ...
            <div className="bg-indigo-500/5 border-l-2 border-indigo-500 p-4 rounded-r-xl text-xs text-slate-700 dark:text-slate-300 leading-relaxed select-text">
              <span className="font-extrabold text-indigo-650 dark:text-indigo-400 block mb-1">💡 거시경제 종합 시각</span>
          ...
          {/* 2. 핵심 브리핑 요약 */}
          <div className="bg-white dark:bg-slate-900 border border-indigo-150 dark:border-indigo-500/10 rounded-2xl p-5 space-y-4 relative overflow-hidden shadow-sm">
            ...
            <div className="space-y-3">
              {aiSummary5Lines.map((line, idx) => (
                <div key={idx} className="flex gap-2.5 items-start">
                  <span className="text-indigo-650 dark:text-indigo-400 text-xs font-black font-mono mt-0.5">{idx + 1}.</span>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-sm">
            ...
            <div className="bg-emerald-500/5 border-l-2 border-emerald-500 p-3.5 rounded-r-xl text-xs text-slate-700 dark:text-slate-300 leading-relaxed max-w-4xl select-text">
              <span className="font-extrabold text-emerald-600 dark:text-emerald-400 block mb-1">💡 시장 마감 한줄 요약</span>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-slate-50/50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-850 shadow-sm">
                <span className="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest block mb-2">US Leader / 미국 주도주</span>
                <div className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-extrabold whitespace-normal break-keep bg-white dark:bg-slate-900/40 p-3 rounded-lg border border-slate-200 dark:border-slate-850/50">
                  {usLeaders}
                </div>
              </div>
              <div className="bg-slate-50/50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-850 shadow-sm">
                <span className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest block mb-2">Featured Stock Movement / 특징주 분석</span>
                <div className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-medium whitespace-pre-wrap break-keep bg-white dark:bg-slate-900/40 p-3 rounded-lg border border-slate-200 dark:border-slate-850/50">
                  {usFeaturedStock}
                </div>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-indigo-50/30 via-slate-50 to-indigo-50/10 dark:from-slate-900 dark:via-slate-900 dark:to-indigo-950/20 border border-indigo-100 dark:border-indigo-500/10 rounded-2xl p-5 space-y-5 shadow-sm">
            <div className="border-b border-slate-200 dark:border-slate-800 pb-2 flex items-center justify-between">
              ...
              <span className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 text-[9px] font-black rounded border border-indigo-200 dark:border-indigo-500/20 uppercase font-mono">
                Sectors Active
              </span>
            </div>

            {/* 수급 이동 시나리오 */}
            <div className="bg-white dark:bg-slate-950/60 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/15 text-xs text-slate-700 dark:text-slate-300 leading-relaxed shadow-sm">
              <h4 className="font-extrabold text-indigo-600 dark:text-indigo-300 mb-1.5">📊 전업 트레이더 전략적 연결 분석</h4>
            ...
            <div className="space-y-3.5">
              <h5 className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider block">⚡ 오늘 장 연동 예상 섹터 및 구성 종목</h5>
                  ...
                  const sentimentColors = 
                    sec.sentiment === 'bullish' 
                      ...
                      : sec.sentiment === 'bearish' 
                        ? 'bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-blue-750 dark:text-blue-400' 
                        : 'bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800/60 text-slate-600 dark:text-slate-450';

                  return (
                    <div key={idx} className="bg-white dark:bg-slate-950/50 border border-slate-200 dark:border-slate-850 p-4 rounded-xl flex flex-col gap-3 hover:border-indigo-150 dark:hover:border-indigo-500/30 hover:shadow-sm transition-all duration-200 shadow-sm">
                      {/* 섹터 헤더 */}
                      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-900/60 pb-2">
                        <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-1 border-t border-slate-100 dark:border-slate-900/40 pt-2.5">
                        {sec.stocks.map((stock, sIdx) => (
                          <span 
                            key={sIdx} 
                            className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-500/5 hover:bg-indigo-100 dark:hover:bg-indigo-500/10 border border-indigo-150 dark:border-indigo-500/15 hover:border-indigo-200 dark:hover:border-indigo-500/35 text-indigo-700 dark:text-indigo-300 font-extrabold text-[10.5px] rounded-md transition-all cursor-default"
                          >
                            {stock}
                          </span>
                        ))}
            ...
            {domesticMatches.length > 0 && (
              <div className="border-t border-slate-200 dark:border-slate-800 pt-3 mt-1.5">
                <span className="text-[9.5px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest block mb-2">📋 연동 주도주 세부 분석 요약</span>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {domesticMatches.map((item, idx) => (
                    <div key={idx} className="bg-slate-50/50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 p-2.5 rounded-lg flex flex-col justify-start gap-1 w-full shadow-sm">
                      <span className="text-xs font-extrabold text-slate-700 dark:text-slate-300 leading-tight">{item.stockName}</span>
                      <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-mono font-bold block">연계: {item.theme}</span>
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 leading-relaxed block break-keep">{item.reason}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-sm">
              ...
              <div className="space-y-3.5">
                {filteredInterestThemes.map((item: any, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-slate-50/50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-850 flex flex-col gap-3.5 shadow-sm">
                      {/* Theme Label */}
                      <div className="flex items-center gap-2 border-b border-slate-250 dark:border-slate-800 pb-2.5 shrink-0 min-h-[36px]">
                      ...
                      <div className="flex flex-wrap gap-2.5">
                        {stocks.map((stock: any, sIdx: number) => {
                          ...
                          return (
                            <div 
                              key={sIdx} 
                              className="inline-flex items-center gap-2 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-250 font-extrabold text-xs px-3.5 py-1.5 rounded-full border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 hover:scale-102 transition-all duration-200 cursor-default select-none shadow-sm"
                            >
          ...
          <div className="bg-white dark:bg-slate-900 border border-red-150 dark:border-red-500/10 rounded-2xl p-5 space-y-3 shadow-sm">
            <h3 className="text-xs font-black text-slate-600 dark:text-slate-400 tracking-wider uppercase flex items-center gap-1.5 border-b border-slate-200 dark:border-slate-800 pb-2">
              <AlertTriangle className="w-4 h-4 text-red-500" />
              <span>오늘의 핵심 주의 리스크 및 위기 요소</span>
            </h3>
            <div className="bg-red-50 dark:bg-red-950/10 border border-red-100 dark:border-red-500/10 p-4 rounded-xl flex items-start gap-2.5">
              <span className="w-5 h-5 rounded-full bg-white dark:bg-slate-950 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20 text-[10px] font-mono font-black flex items-center justify-center flex-shrink-0 mt-0.5">
                !
              </span>

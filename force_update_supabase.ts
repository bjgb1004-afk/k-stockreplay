...
async function forceUpdate() {
  ...
  console.log(`[Force Update] Injecting high-quality 7/31 content using Service Key...`);
  ...
  const { error: e1 } = await supabase.from('kstock_platform_data').upsert({
    key: `afternoon_report_${date}`,
    date_kst: date,
    data: reportData
  }, { onConflict: 'key,date_kst' });

  const { error: e2 } = await supabase.from('kstock_platform_data').upsert({
  ...
  if (e1 || e2) {
    console.error('Update Failed:', e1 || e2);
  } else {
    console.log('Successfully updated 7/31 data in DB.');
  }
}
forceUpdate();
